...
func enemy_float():
	var enemy_new_pos = enemy_orig_position + Vector2(randf_range(-20,20), randf_range(-100, 100))
...
func shoot_rocket():
	...
	rocket.direction = Vector2.RIGHT.rotated(randf_range(-0.6, 0.6))
...
func shoot_bullet():
	...
	bullet.direction = Vector2.RIGHT.rotated(randf_range(-0.05, 0.05))
	bullet.position = $PlayerShip/Muzzleflash.global_position
	add_child(bullet)

func shoot_enemy_bullet():
	var bullet = BulletEnemy.instantiate()
	bullet.direction = Vector2.LEFT.rotated(randf_range(-0.05, 0.05))
